...
$$("#add-user").onclick = function () {
    ...
    axios.post('http://localhost/day25(1009)/demo/lib/sever.php?fn=add', data).then(res => {
        // console.log(res);
        if (res == 1) {
            // 插入成功后,关闭弹出框,刷新页面
            // console.log('success');
...
/****实现数据列表 *****/
function list() {
    axios.get('http://localhost/day25(1009)/demo/lib/sever.php?fn=sel').then(res => {
        //将json转化为对象
        res = JSON.parse(res);
        // console.log(res);
        //遍历数据，追加到页面中
...
$$('#del-user').onclick = function () {
    ...
    // 2 发送ajax删除
    axios.get('http://localhost/day25(1009)/demo/lib/sever.php?fn=del&id=' + id).then(res => {
        if (res == 1) {
            // 删除成功,关闭模态框,刷新页面
